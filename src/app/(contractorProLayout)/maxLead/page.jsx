...
export default function MaxLead() {
  ...
  const subCategoryOptions = selectedCategories?.map((category) => {
    const subCategories = allCategory?.data?.find((service) => service.category === category.value)?.subCategory;
    return {
  ...
  const prepareDataToSend = async() => {
    const servicesYouProvide = selectedCategories.map((category) => category.value);
      ...
      <div className="max-w-4xl mx-auto p-6 bg-white">
        ...
        <div className="mb-8">
          ...
          {subCategoryOptions?.map((categoryOption) => (
            <div key={categoryOption.category}>
              <h3 className="text-lg font-semibold mb-4">{categoryOption.category}</h3>
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    onChange={(selectedSubOptions) => handleSubcategoryChange(categoryOption.category, selectedSubOptions)}
                    getOptionLabel={(e) => e.label}
                    getOptionValue={(e) => e.value}
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <div className="bg-blue-100 rounded-lg p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <button onClick={prepareDataToSend} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded-lg transition-colors focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 outline-none">
              See Leads
            </button>
          </div>
