...
import { selectCurrentContractor, updateContractorData } from "../../../redux/features/contractor/contractorSlice";
          ...
          <div className="flex-1 space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-8 max-w-3xl mx-auto">
                <button
                  type="button"
                  className={`text-sm border border-black px-3 py-1 rounded-2xl ${noOfEmployee === 'Self-employed' ? 'bg-blue-600 text-white' : ''}`}
                ...
                <button
                  type="button"
                  className={`text-sm border border-black px-3 py-1 rounded-2xl ${noOfEmployee === '2-10' ? 'bg-blue-600 text-white' : ''}`}
                ...
                <button
                  type="button"
                  className={`text-sm border border-black px-3 py-1 rounded-2xl ${noOfEmployee === '11-50' ? 'bg-blue-600 text-white' : ''}`}
                ...
                <button
                  type="button"
                  className={`text-sm border border-black px-3 py-1 rounded-2xl ${noOfEmployee === '51-200' ? 'bg-blue-600 text-white' : ''}`}
                ...
                <button
                  type="button"
                  className={`text-sm border border-black px-3 py-1 rounded-2xl ${noOfEmployee === '200+' ? 'bg-blue-600 text-white' : ''}`}
                  onClick={() => handleCompanySize('200+')}
                >
                  200+
                </button>
              </div>
              <input type="hidden" {...register("noOfEmployee", { required: "Company size is required" })} value={noOfEmployee} />
              {errors.noOfEmployee && (
                <p className="text-red-500 text-sm">{errors.noOfEmployee.message}</p>
              )}
            </div>

            <div>
              <button type="submit" className="bg-blue-600 py-2 text-white w-full rounded-xl">
                Continue
              </button>
            </div>
          </div>
